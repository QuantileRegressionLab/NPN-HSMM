...
grids = list(seq(7,17,length.out=100), seq(10,19,length.out=100), seq(16,46,length.out=100))
...
for(sim in 1:nsim){
  ...
  data_gen <- hsmm.multi.gen(ns = N[a], P = P, K = K, m = S, delta = init_sim, gamma = gamma_sim, 
                             mu = mu, rho = sigma_sim, d = d_true[[b]], error = error[c])
  Y = data_gen$series
  ...
  ICL_HSMM = foreach(i=1:length(grid), .combine=c, .packages = c("mvtnorm", "mclust", "mhsmm", "car", "glasso")) %dopar% {
    aaa = try(EM_HSMM(Y = Y, S = S, Par = Par_HSMM, M = M, sojourn.distribution = sojourn[b], lambda = grid[i], pen_EBIC = 0.5, seed = 1, err = 1e-4, iterMax = 5e2))
  ...
  aaa = EM_HSMM(Y = Y, S = S, Par = Par_HSMM, M = M, sojourn.distribution = sojourn[b], lambda = lambda_HSMM, pen_EBIC = 0.5, seed = 1, err = 1e-4, iterMax = 5e2)
  ...
  while(ARI_HSMM < 0.3 & iter < 10){
    ...
    ccc = EM_HSMM(Y = Y, S = S, Par = Par_HSMM, M = M, sojourn.distribution = sojourn[b], lambda = lambda_HSMM, pen_EBIC = 0.5, seed = 1, err = 1e-4, iterMax = 5e2)
    if(ccc$loglik > llk){
      aaa = ccc
      llk = ccc$loglik
      ARI_HSMM = adjustedRandIndex(apply(aaa$u, 1, which.max), state)
...
for (sim in 1:nsim) {
  ...
  rho = sapply(1:S, function(s) as.matrix(forceSymmetric(cov2cor(rho[,,s]))), simplify = F)
  sigma = sapply(1:S, function(s) cov2cor(as.matrix(nearPD(solve(rho[[s]]))$mat)), simplify = F)
  ...
  results_list.tmp = foreach(r = 1:R, .packages = c("mvtnorm", "mclust", "mhsmm", "car", "glasso")) %dopar% {
    print(r)
    set.seed(r)
    
    data_boot = boot.hsmm.multi.gen(ns = N[a], P = P, K = K, m = S, delta = delta, gamma = gamma, rho = sigma, d = d)
    ...
    fit.boot = try(EM_HSMM(Y = Y_boot, S = S, Par = Par_HSMM, M = M, sojourn.distribution = sojourn[b], lambda = lambda_HSMM, pen_EBIC = 0.5, seed = 1, err = 1e-4, iterMax = 5e2))
...
####################
coverage.summary(fit = results_list, boot = results_list.boot, theta = theta, d_true = d_true, gamma_sim = gamma_sim, sojourn.distribution = "poisson", alpha = 0.05)
...
####################
coverage.summary(fit = results_list, boot = results_list.boot, theta = theta, d_true = d_true, gamma_sim = gamma_sim, sojourn.distribution = "nbinom", alpha = 0.05)
