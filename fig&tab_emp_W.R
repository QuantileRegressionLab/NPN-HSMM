...
for (j in 1:S) {
  ...
  g[[j]] <- graph_from_adjacency_matrix(as.matrix(M.parcorr[[j]]), mode="upper", weighted=TRUE, diag=FALSE) #grafo pesato
  g_adj[[j]] <- graph_from_adjacency_matrix(as.matrix(Adj_mmdl[,,j]), mode="upper", weighted=F, diag=FALSE) #grafo non pesato
  e[[j]] <- as_edgelist(g[[j]])
  ...
  df.e.b[[j]] <- as.data.frame(cbind(e[[j]], 10*(betw[[j]]/max(betw[[j]])) ))
...
qgraph(df.e[[1]], ######si aggiunge e toglie la legenda ora solo inserendo legend=T
...
qgraph(df.e.b[[1]], ######si aggiunge e toglie la legenda ora solo inserendo legend=T
       layout = "spring", # Senza questo le variabili sono disposte a cerchio
       fade=F,  # Fa sì che i nodi non siano trasparenti
...
qgraph(df.e.b[[3]], ######si aggiunge e toglie la legenda ora solo inserendo legend=T
       layout = "spring", # Senza questo le variabili sono disposte a cerchio
       fade=F,  # Fa sì che i nodi non siano trasparenti
       directed = F, # Directed graphs
...
library(reshape2)
library(gridExtra)
date <- as.Date(rownames(prices_mat))
N <- dim(prices_mat)[1]
prices <- prices_mat
prices_norm <- apply(prices, 2, normalTS)

price_crypto_df <- data.frame(date,
                              prices_norm[,1], prices_norm[,2], prices_norm[,3], prices_norm[,4], prices_norm[,5])

price_stock_df <- data.frame(date,
                             prices_norm[,6], prices_norm[,7])

price_currency_df <- data.frame(date,
                                prices_norm[,8], prices_norm[,9], prices_norm[,10], prices_norm[,11], prices_norm[,12])

price_energy_df <- data.frame(date,
                              prices_norm[,13], prices_norm[,14], prices_norm[,15], prices_norm[,16], prices_norm[,17])

price_macroeconomy_df <- data.frame(date,
                                    prices_norm[,18], prices_norm[,19], prices_norm[,20], prices_norm[,21])
...
gg1 <- melt(price_crypto_df, id = c("Date"), variable.name="Cryptocurrency", value.name="Price")
gg2 <- melt(price_stock_df, id = c("Date"), variable.name="Stock", value.name="Price")
gg3 <- melt(price_currency_df, id = c("Date"), variable.name="Currency", value.name="Price")
gg4 <- melt(price_energy_df, id = c("Date"), variable.name = "Energy", value.name = "Price")
gg5 <- melt(price_macroeconomy_df, id = c("Date"), variable.name="Macroeconomy", value.name="Price")
...
p5 <- ggplot(gg5, aes(x=Date, y=Price, color=Macroeconomy))+
  ...
  geom_point(alpha = 0)+ geom_line() +
  scale_x_date(date_labels = c("%Y"), breaks = "1 year") + xlab("Date")
