...
library(parallel)
library(doParallel)
load("analisi_empirica_2023_M30_R20_rev.RData")
...
results_list.boot = foreach(r = 1:R, .packages = c("mvtnorm", "mclust", "mhsmm", "car", "glasso")) %dopar% {
  print(r)
  set.seed(r)
  
  data_boot = boot.hsmm.multi.gen(ns = N, P = P, K = K, m = S, delta = delta, gamma = gamma, rho = sigma, d = d)
  ...
  fit.boot = try(EM_HSMM(Y = Y_boot, S = S, Par = Par_HSMM, M = M, sojourn.distribution = "np", lambda = lambda_HSMM, pen_EBIC = 0.5, seed = 1, err = 1e-4, iterMax = 5e2))
  if(inherits(fit.boot, "try-error")) fit.boot = NULL
  fit.boot
}

####################
save.image("analisi_empirica_2023_M30_R20_boot_rev.RData")
####################
